...
export function ExpiryRow({ record }: { record: Item }) {
  ...
  const meta = statusMeta(colors)[record.status];
      ...
      <View style={styles.recordMain}>
        ...
        {record.status !== 'Pendente' && (
          <View style={[styles.statusTag, { backgroundColor: meta.bg }]}>
            <StatusIcon status={record.status} size={11} color={meta.text} />
            <Text style={[styles.statusTagText, { color: meta.text }]}>{record.status}</Text>
          </View>
        )}
      </View>
